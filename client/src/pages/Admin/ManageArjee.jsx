    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-8">
        <div>
           <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-black text-[#0A1128] tracking-tighter uppercase italic">Ritual <span className="text-orange-600 not-italic">Vault</span></h1>
           </div>
        ...
        <div className="flex items-center gap-4 bg-white px-6 py-3.5 rounded-[28px] border border-slate-200 shadow-sm">
           <div className="flex flex-col border-r border-slate-100 pr-6">
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Vault Status</span>
              <span className="text-[11px] font-black text-[#0A1128] uppercase tracking-tighter">Active Sync</span>
           </div>
           <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-orange-50 text-orange-600 flex items-center justify-center font-black text-xs">
                 {videos.length}
              </div>
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ritual Proofs</span>
           </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
        ...
        <div className="xl:col-span-4">
           <div className="bg-white p-10 rounded-[48px] border border-slate-200/60 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.03)] h-fit xl:sticky xl:top-10">
              <div className="flex justify-between items-center mb-8">
                 <div>
                    <h2 className="text-xl font-black text-[#0A1128] tracking-tighter uppercase italic">Upload <span className="text-orange-600 not-italic">Studio</span></h2>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Publish sacred proofs</p>
                 </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Proof Title</label>
                  <input type="text" required placeholder="e.g. Swamani Ritual 12th May" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[20px] outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 font-black text-[13px] text-[#0A1128] transition-all" />
                </div>

                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Service Category</label>
                  <select value={form.category} onChange={e => setForm({...form, category: e.target.value})} className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-[20px] outline-none font-black text-[10px] uppercase tracking-[0.2em] text-[#0A1128] focus:border-orange-500 transition-all cursor-pointer">
                ...
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Social Integration (Insta Link)</label>
                  <div className="relative">
                    <input type="text" placeholder="https://instagram.com/reels/..." value={form.instagramUrl} onChange={e => setForm({...form, instagramUrl: e.target.value})} className="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-100 rounded-[20px] outline-none focus:border-orange-500 font-bold text-[11px] text-slate-600 transition-all" />
                    <FaInstagram className="absolute left-5 top-1/2 -translate-y-1/2 text-pink-500" size={16} />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Native Vertical Video</label>
                  <input type="file" accept="video/*" onChange={e => setVideoFile(e.target.files[0])} className="hidden" id="reel-upload" />
                  <label htmlFor="reel-upload" className="w-full p-8 border-2 border-dashed border-slate-100 rounded-[24px] flex flex-col items-center gap-3 cursor-pointer hover:bg-slate-50 hover:border-orange-200 transition-all group overflow-hidden relative">
                ...
                <button type="submit" disabled={saving || (!videoFile && !form.instagramUrl)} className="w-full py-4.5 bg-[#0A1128] text-white rounded-[24px] font-black text-[11px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-2xl shadow-slate-300 active:scale-95 disabled:opacity-50">
        ...
        <div className="xl:col-span-8">
              ...
              {videos.map((vid, i) => {
                 ...
                 return (
                  <motion.div 
                    ...
                    key={vid._id} 
                    className="relative aspect-[9/16] rounded-[40px] overflow-hidden bg-[#0A1128] group shadow-2xl border border-slate-200/20"
                  >
                    {vid.instagramUrl ? (
                      <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-tr from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] text-white p-6 relative">
                         ...
                         <span className="text-[10px] font-black text-white uppercase tracking-[0.3em] relative z-10 text-center leading-relaxed">Integrated Instagram Reel</span>
                      </div>
                    ) : isLocal ? (
                    ...
                    <div className="absolute top-6 left-6 right-6 flex justify-between items-start opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-[-10px] group-hover:translate-y-0">
                       <span className="bg-orange-600 text-white text-[8px] font-black uppercase px-4 py-2 rounded-full tracking-widest shadow-2xl shadow-orange-500/20">{vid.category}</span>
                    ...
                    <div className="absolute bottom-6 left-6 right-6 text-center transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                       <h3 className="text-[12px] font-black text-white uppercase tracking-tighter truncate px-2 mb-2">{vid.title}</h3>
           ...
           {videos.length === 0 && !loading && (
              <div className="py-40 text-center bg-white rounded-[48px] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center">
                 <FaHistory size={48} className="text-slate-100 mb-6" />
                 <p className="text-[11px] font-black text-slate-300 uppercase tracking-[0.4em]">Vault holds no ritual proofs</p>
              </div>
           )}
        </div>
      </div>
