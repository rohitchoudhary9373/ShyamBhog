...
export default function Refunds() {
  ...
  if (loading) return (
    <div className="py-40 text-center flex flex-col items-center justify-center gap-4">
       ...
       <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] animate-pulse">Auditing Dispute Ledger...</p>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
           <div className="flex items-center gap-2 mb-1.5">
             ...
             <span className="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em] italic">Sacred Treasury Operations</span>
           </div>
           <h1 className="text-3xl font-black text-[#0A1128] tracking-tight uppercase italic">Refund Governance</h1>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
           <button 
             onClick={exportCSV}
             className="flex-1 lg:flex-none bg-white border border-slate-200 text-[#0A1128] px-5 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:border-orange-500 hover:text-orange-600 transition-all shadow-sm flex items-center justify-center gap-2 active:scale-95"
           >
           ...
           <button 
             ...
             className="flex-1 lg:flex-none bg-[#0A1128] text-white px-6 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg flex items-center justify-center gap-2 active:scale-95"
           >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 md:gap-6">
        {[
          ...
          <div key={idx} className="bg-white p-5 rounded-[28px] border border-slate-200/60 shadow-sm relative overflow-hidden group flex flex-col justify-between h-28">
      ...
      {/* ── FILTERS ── */}
      <div className="flex flex-col xl:flex-row items-center gap-4 bg-white p-4 rounded-[28px] border border-slate-200/60 shadow-sm sticky top-4 z-30">
         <div className="flex items-center gap-3 bg-slate-50 px-4 py-3 rounded-2xl flex-1 w-full border border-slate-100/55">
            ...
            <input
              ...
              className="bg-transparent border-none outline-none font-bold text-xs text-[#0A1128] placeholder:text-slate-300 w-full"
              value={searchQuery}
         ...
         <div className="flex flex-wrap items-center gap-2.5 w-full xl:w-auto">
            ...
            <div className="flex-1 sm:flex-none">
              <select
                 className="w-full sm:w-auto bg-slate-50 border border-slate-100 outline-none font-black text-[9px] text-[#0A1128] px-4 py-3 rounded-xl uppercase tracking-widest cursor-pointer hover:border-orange-500 transition-all"
                 value={statusFilter}
            ...
            {isSuperAdmin && (
              <div className="flex items-center gap-2 bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-100 min-w-[150px] w-full sm:w-auto">
                 <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">Source:</span>
                 <select 
                   value={selectedTenant} 
                   onChange={(e) => setSelectedTenant(e.target.value)}
                   className="bg-transparent border-none outline-none font-black text-[9px] text-orange-600 uppercase tracking-widest cursor-pointer focus:ring-0 w-full"
                 >
        ...
        {filteredRefunds.map((refund, i) => {
          ...
          return (
            <motion.div 
              ...
              key={refund._id}
              className="bg-white border border-slate-200/60 rounded-[32px] p-6 shadow-sm hover:shadow-xl hover:shadow-orange-100/10 hover:border-orange-200/40 transition-all duration-300 relative group overflow-hidden"
            >
              {/* Card Header */}
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pb-5 border-b border-slate-100/80 mb-5">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 text-[#0A1128] flex items-center justify-center font-black shadow-sm group-hover:scale-105 transition-transform shrink-0">
                    <FaBoxOpen size={20} className="text-slate-400 group-hover:text-orange-500 transition-colors" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2.5">
                      <span className="font-black text-[#0A1128] text-base tracking-tighter uppercase italic">
                ...
                <div className="flex items-center gap-3 w-full lg:w-auto justify-between lg:justify-end">
                  <div className="text-left lg:text-right">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Disputed Sum</p>
                    <p className="text-2xl font-black text-orange-600 tracking-tight leading-none">₹{refund.amount}</p>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div className="bg-slate-50/50 border border-slate-100/80 p-5 rounded-[22px] flex flex-col justify-between">
                  <div>
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-2.5">Devotee Information</span>
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-orange-600 text-white flex items-center justify-center font-black text-sm uppercase">
                        {refund.userId?.name?.charAt(0) || 'D'}
                      </div>
                      <div>
                        <p className="font-black text-[#0A1128] text-xs uppercase tracking-tight leading-none mb-1">{refund.userId?.name || 'Devotee User'}</p>
                        <p className="text-[9px] font-bold text-slate-500 tracking-wider">{refund.userId?.mobile || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-slate-400">
                ...
                <div className="bg-slate-50/50 border border-slate-100/80 p-5 rounded-[22px] flex flex-col justify-between">
                  <div>
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-2.5">Refund Settlement Method</span>
                    {refund.upiId ? (
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">BHIM UPI</p>
                          <p className="text-[11px] font-black text-[#0A1128] tracking-wider truncate max-w-[140px]">{refund.upiId}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Direct Bank Account</p>
                          <p className="text-[11px] font-black text-[#0A1128] tracking-widest">****{refund.bankDetails.accountNumber.slice(-4)}</p>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Internal Ledger Account</p>
                          <p className="text-[11px] font-black text-[#0A1128] uppercase tracking-widest">Devotee Wallet</p>
                        </div>
                      </div>
                    )}
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-slate-400">
                    ...
                    {refund.receiptNumber ? (
                      <span className="text-emerald-600 font-black">{refund.receiptNumber}</span>
                    ) : (
                ...
                <div className="bg-slate-50/50 border border-slate-100/80 p-5 rounded-[22px] flex flex-col justify-between">
                  <div>
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Dispute Grounds</span>
                  ...
                  {refund.adminRemarks && (
                    <div className="mt-2.5 pt-2 border-t border-slate-200/60 border-dashed">
                      <span className="text-[8px] font-black text-orange-600 uppercase tracking-widest block mb-0.5">Resolution Note</span>
              ...
              <div className="bg-slate-50/30 border border-slate-100/60 p-4 rounded-[22px] mb-5">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-[9px] font-black uppercase tracking-widest text-slate-400">
                  ...
                  <div className="flex flex-wrap gap-4 sm:gap-6 w-full sm:w-auto">
                    {[
                      ...
                      <div key={idx} className="flex items-center gap-1.5">
                        ...
                        <span className={step.active ? 'text-slate-800 font-black' : 'text-slate-400 font-semibold'}>{step.label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  {refund.orderId && (
                    <button 
                      ...
                      className="px-4 py-2 bg-white border border-slate-200 hover:border-[#0A1128] hover:text-[#0A1128] rounded-xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-sm flex items-center gap-1.5"
                    >
                      <FaBoxOpen size={10} />
                      View Order
                    </button>
                  ...
                  {refund.receiptNumber && (
                    <button 
                      ...
                      className="px-4 py-2 bg-orange-50 text-orange-600 hover:bg-orange-600 hover:text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 border border-orange-100 flex items-center gap-1.5"
                    >
                  ...
                  {isPending ? (
                    <>
                      <button 
                        ...
                        className="px-4 py-2 bg-emerald-600 text-white hover:bg-emerald-700 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-emerald-100 flex items-center gap-1.5"
                      >
                        <FaWallet size={10} />
                        Settle Dispute
                      </button>
                      <button 
                        ...
                        className="px-4 py-2 bg-red-50 text-red-600 hover:bg-red-500 hover:text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 border border-red-100 flex items-center gap-1.5"
                      >
                    ...
                    <div className="flex items-center gap-2 text-[9px] font-black text-slate-400 uppercase tracking-widest px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg">
      ...
      {!loading && filteredRefunds.length === 0 && (
         <div className="py-40 text-center flex flex-col items-center justify-center gap-6 opacity-20">
            ...
            <div className="space-y-1">
               <p className="text-sm font-black text-[#0A1128] uppercase tracking-tight">No Refund Requests</p>
      ...
      <AnimatePresence>
        {processingRefund && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-[#0A1128]/35 backdrop-blur-md" onClick={() => setProcessingRefund(null)}></div>
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="relative z-10 w-full max-w-lg bg-white rounded-[40px] shadow-2xl overflow-hidden border border-slate-200/60"
            >
              <header className="p-8 bg-[#0A1128] text-white flex justify-between items-center relative overflow-hidden">
                <div className="absolute top-0 right-0 p-6 opacity-10 text-6xl -mr-6 -mt-6"><FaShieldAlt /></div>
                <div className="relative z-10">
                  <h3 className="text-xl font-black tracking-tighter uppercase italic">Settle Dispute</h3>
              ...
              <form onSubmit={handleProcessSubmit} className="p-8 space-y-6 bg-[#F8FAFC]">
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block px-1">Dispute Action Decided</label>
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      type="button"
                      onClick={() => setProcessForm({ ...processForm, status: 'approved' })}
                      className={`py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest border transition-all ${
                ...
                {processForm.status === 'approved' && (
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block px-1">Disbursement Settle Method</label>
                    <div className="grid grid-cols-3 gap-2.5">
                      {[
                        ...
                        <button
                          ...
                          className={`py-3 rounded-xl font-black text-[9px] uppercase tracking-widest border flex flex-col items-center justify-center gap-1.5 transition-all ${
                ...
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block px-1">Resolution Remarks Note</label>
                  <textarea
                    required
                    ...
                    rows={4}
                    className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-orange-500/10 focus:border-orange-500 font-medium text-xs text-[#0A1128] resize-none transition-all placeholder:text-slate-300"
                  />
                </div>
                ...
                <div className="flex gap-4 pt-4 border-t border-slate-100">
                  <button
                    type="button"
                    onClick={() => setProcessingRefund(null)}
                    className="flex-1 py-4 border border-slate-200 rounded-2xl font-black text-[10px] uppercase tracking-widest text-slate-400 hover:bg-white hover:text-[#0A1128] transition-all"
                  >
                    Abort
                  </button>
                  <button
                    type="submit"
                    className="flex-[2] py-4 bg-[#0A1128] text-white hover:bg-orange-600 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl active:scale-95"
                  >
      ...
      <AnimatePresence>
        {viewingOrder && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-[#0A1128]/35 backdrop-blur-md" onClick={() => setViewingOrder(null)}></div>
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="relative z-10 w-full max-w-xl bg-white rounded-[40px] shadow-2xl overflow-hidden border border-slate-200/60 flex flex-col max-h-[85vh]"
            >
              <header className="p-8 bg-[#0A1128] text-white flex justify-between items-center shrink-0">
                <div>
                  <h3 className="text-xl font-black tracking-tighter uppercase italic">Booking Invoice Details</h3>
                  <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mt-0.5">
              ...
              <div className="p-8 overflow-y-auto custom-scrollbar space-y-6 bg-[#F8FAFC] flex-grow">
                {/* Devotee Details */}
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-3">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block border-b border-slate-50 pb-2">Devotee Record</span>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Name</p>
                      <p className="text-xs font-black text-slate-800 uppercase mt-0.5">{viewingOrder.name}</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">WhatsApp</p>
                      <p className="text-xs font-black text-slate-800 mt-0.5">{viewingOrder.whatsapp}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-4">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block border-b border-slate-50 pb-2">Ordered Services</span>
                  <div className="space-y-3">
                    {viewingOrder.items?.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-start gap-4 p-3 bg-slate-50 rounded-xl border border-slate-100/50">
                        <div>
                          <p className="font-black text-xs text-[#0A1128] uppercase">{item.title || 'Service Offering'}</p>
                          {item.slot && (
                        ...
                        <div className="text-right shrink-0">
                          <p className="text-[9px] font-black text-[#0A1128]">₹{item.price}</p>
                ...
                {/* Payment Breakdown */}
                <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-3">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block border-b border-slate-50 pb-2">Financial Breakdown</span>
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="flex justify-between font-black text-slate-900 border-t border-slate-50 pt-2 text-sm">
      ...
      <AnimatePresence>
        {viewingReceipt && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-[#0A1128]/35 backdrop-blur-md" onClick={() => setViewingReceipt(null)}></div>
            <motion.div 
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="relative z-10 w-full max-w-lg bg-white rounded-[40px] shadow-2xl overflow-hidden border border-slate-200/60"
            >
              <div id="refund-receipt-printable" className="p-8 space-y-8 bg-white text-slate-800">
                {/* Receipt Header */}
                <div className="flex justify-between items-start border-b-2 border-slate-100 pb-5">
                  <div>
                    <h2 className="text-2xl font-black text-[#0A1128] tracking-tighter uppercase italic leading-none">SHYAM BHOG</h2>
                    <span className="text-[7.5px] font-black text-orange-600 uppercase tracking-[0.3em] block mt-1.5">Official Settle Receipt</span>
                  </div>
                  <div className="text-right">
                    <span className="bg-emerald-50 text-emerald-600 border border-emerald-100 text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-wider">
                      Disbursed
                    </span>
                    <p className="text-[9.5px] font-black text-[#0A1128] uppercase mt-2.5">{viewingReceipt.receiptNumber}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-xs border-b border-slate-100 pb-6">
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Receipt Date</p>
                  ...
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Transaction Ref</p>
                    <p className="font-bold text-[#0A1128] uppercase">{viewingReceipt.orderId?.paymentId || 'Wallet Ledger'}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Original Invoice</p>
                    <p className="font-bold text-[#0A1128] uppercase">#{viewingReceipt.orderId?._id?.slice(-8).toUpperCase()}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Devotee Customer</p>
                    <p className="font-bold text-[#0A1128] uppercase">{viewingReceipt.userId?.name}</p>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 space-y-4">
                  <div className="flex justify-between items-center text-xs">
                    <div>
                      <p className="font-black text-[#0A1128] uppercase">Sacred Order Dispute Settlement</p>
                      <p className="text-[8.5px] text-slate-400 mt-0.5">Refund type: {viewingReceipt.refundMethod?.toUpperCase() || 'WALLET'}</p>
                    </div>
                    <span className="font-black text-base text-emerald-600">₹{viewingReceipt.amount}</span>
                  </div>
                </div>
                ...
                <div className="bg-orange-50/50 border border-orange-100/50 rounded-2xl p-4 text-[10.5px] leading-relaxed text-[#0a1128]/80 font-medium">
                  <span className="text-[8px] font-black text-orange-600 uppercase tracking-widest block mb-1">Governance Resolution Remarks</span>
                ...
                <div className="text-center space-y-1 pt-4 border-t border-slate-100">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none">Jai Shree Shyam</p>
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-4 shrink-0 justify-end">
                <button
                  ...
                  className="px-5 py-2.5 border border-slate-200 rounded-xl font-black text-[9px] uppercase tracking-widest text-slate-400 hover:bg-white hover:text-slate-700 transition-all active:scale-95"
                >
                  Close
                </button>
                <button
                  ...
                  className="px-5 py-2.5 bg-[#0A1128] text-white hover:bg-orange-600 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all active:scale-95 shadow-lg flex items-center gap-1.5"
                >
                  <FaPrint size={10} />
                  Print Receipt
                </button>
