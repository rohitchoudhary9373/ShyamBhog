...
export default function LoginHistory() {
  ...
  if (loading) return (
    <div className="py-40 text-center flex flex-col items-center justify-center gap-4">
       ...
       <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] animate-pulse">Syncing Security Telemetry...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-8">
        <div>
           <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-black text-[#0A1128] tracking-tighter uppercase italic">Security <span className="text-orange-600 not-italic">Audit</span></h1>
           </div>
        ...
        <div className="flex items-center gap-4">
           <div className="px-6 py-3 bg-emerald-50 text-emerald-600 rounded-[22px] text-[10px] font-black uppercase tracking-[0.2em] border border-emerald-100 flex items-center gap-3 shadow-sm">
        ...
        <div className="divide-y divide-slate-50">
          ...
          {history.map((log, idx) => (
            <motion.div 
               ...
               key={idx} 
               className="p-10 flex flex-col xl:flex-row items-center justify-between hover:bg-slate-50/40 transition-all duration-300 group gap-8"
            >
              <div className="flex items-center gap-8 flex-1 w-full xl:w-auto">
                <div className="w-16 h-16 rounded-[24px] bg-[#0A1128] text-white flex items-center justify-center shadow-xl shadow-slate-200 group-hover:scale-105 transition-transform duration-300 relative overflow-hidden shrink-0">
                ...
                <div className="space-y-2 flex-1 min-w-0">
                  <div className="flex items-center gap-4">
                     <p className="font-black text-[#0A1128] text-[16px] tracking-tighter uppercase italic">{log.ip || 'ANONYMOUS_IP'}</p>
              ...
              <div className="flex items-center gap-10 w-full xl:w-auto justify-between xl:justify-end border-t xl:border-t-0 pt-6 xl:pt-0 border-slate-100">
                <div className="flex flex-col xl:items-end gap-1">
                   <div className="flex items-center gap-2 text-slate-400">
                      <FaClock size={10} />
                      <p className="text-[11px] font-black text-[#0A1128] tracking-widest">{new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>
                   </div>
                ...
                <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                   ...
                   <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100">
                      ...
                      <span className="text-[9px] font-black text-[#0A1128] uppercase tracking-widest">Verified</span>
                   </div>
                </div>
              </div>
            </motion.div>
          ...
          {history.length === 0 && (
            <div className="py-40 text-center flex flex-col items-center justify-center gap-6 opacity-20">
               ...
               <div className="space-y-1">
                  <p className="text-[14px] font-black text-[#0A1128] uppercase tracking-tighter italic">No security records detected</p>
      ...
      <footer className="flex items-center justify-center gap-3 opacity-30 pt-10">
         <FaShieldAlt size={12} />
         <p className="text-[9px] font-black uppercase tracking-[0.4em]">End-to-End Encryption Protocol Active</p>
      </footer>
    </div>
  );
}
