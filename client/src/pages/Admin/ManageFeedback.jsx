...
export default function ManageFeedback() {
  ...
  if (loading) return (
    <div className="py-40 text-center flex flex-col items-center justify-center gap-4">
       ...
       <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] animate-pulse">Loading Reviews...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
           <h1 className="text-3xl font-black text-[#0A1128] tracking-tight">Testimonials</h1>
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
           {isSuperAdmin && (
             <div className="hidden sm:flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Storefront:</span>
                <select 
           ...
           <button 
             ...
             className="flex-1 lg:flex-none bg-orange-600 text-white px-6 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-[#0A1128] transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2"
           >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        {[
          ...
          <div key={idx} className="bg-white p-5 md:p-6 rounded-[24px] border border-slate-200/60 shadow-sm">
             <div className="flex items-center justify-between mb-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[14px] bg-${s.color}-50 text-${s.color}-600`}>
                   {s.icon}
                </div>
                <span className="text-xl font-black text-[#0A1128] tracking-tight">{s.val}</span>
             </div>
      ...
      <AnimatePresence>
        {showAddForm && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="bg-white rounded-[32px] border border-slate-200 p-8 shadow-xl shadow-slate-200/20"
          >
            <div className="flex items-center gap-3 mb-8">
               <div className="w-8 h-8 rounded-lg bg-orange-50 text-orange-600 flex items-center justify-center">
                  <FaPlus size={12} />
               </div>
               <h3 className="text-lg font-black text-[#0A1128] tracking-tight">New Testimonial</h3>
            </div>
            
            <form onSubmit={handleAddFeedback} className="space-y-6">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                     <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Devotee Name</label>
                     <input
                       type="text"
                       required
                  ...
                  <div className="space-y-2">
                     <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">WhatsApp / Mobile (Optional)</label>
                     <input
                       type="text"
               ...
               <div className="flex justify-end">
                  <button
                     disabled={submitting}
                     className="w-full md:w-auto px-10 py-4 bg-orange-600 text-white rounded-[20px] font-black text-[11px] uppercase tracking-widest shadow-xl shadow-orange-100 hover:bg-[#0A1128] transition-all active:scale-95"
                  >
      ...
      <div className="space-y-4">
         <div className="flex items-center gap-3 px-2 mb-2">
            <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">All Testimonials</h3>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/30 border-b border-slate-100">
                    ...
                    {isSuperAdmin && <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Verified Status</th>}
                    <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Review</th>
                    ...
                    >
                      <td className="p-6">
                         <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-slate-100 text-[#0A1128] flex items-center justify-center font-black text-sm border-2 border-white shadow-sm shrink-0">
                               {f.name.charAt(0)}
                            </div>
                            <div>
                               <div className="flex items-center gap-1.5">
                                  <p className="font-bold text-[#0A1128] text-[13px] tracking-tight leading-none">{f.name}</p>
            ...
            {feedbacks.length === 0 && (
               <div className="py-32 text-center flex flex-col items-center justify-center gap-4 opacity-30">
                  ...
                  <div className="space-y-1">
                     <p className="text-sm font-black text-[#0A1128] uppercase tracking-tight">No Reviews Found</p>
